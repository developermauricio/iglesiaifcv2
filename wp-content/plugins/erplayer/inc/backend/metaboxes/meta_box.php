<?php
/**
 * @package  WP MetaBoxes
 * @version  2020.08.13
 * The opular API to add custom fields.
 * 
 * [x] 2020 August 13 Updated for WordPress 5.5 compatibility
 * [x] 2020 March 19 updated metaboxes-script.js replaced .click with .on
 * [x] 2020 March 19 updated metaboxes-script.js added "use strict"
 * [x] 2020 March 19 removed Chosen script L615-622
 * [x] 2020 March 19 deleted js/chosen.js
 * 
 * [x] 2020 January 23 - Added conditional fields
 * ----
 * [x] 2020 January 21 - Improved sanitarization
 * [x] 2020 January 21 - Improved functions namings
 */


function metaboxes_custom_dir(){
...
function metaboxes_custom_meta_box_field( $field, $meta = null, $repeatable = null ) {
	...
	switch( $type ) {
		...
		break;
		case 'category':
			echo wp_dropdown_categories( array( 'post_type' => $posttype, 
				'posts_per_page' => -1, 
				'orderby' => 'name', 
				'order' => 'ASC' , 
				'echo' => 0 , 
		...
		// image
		case 'image':
			$image = metaboxes_custom_dir().'/images/image.png';  
		...
		case 'repeatable':
			...
			foreach( $meta as $row ) {
				...
				foreach ( $repeatable_fields as $repeatable_field ) {
					...
					metaboxes_custom_meta_box_field( $repeatable_field, $meta[$i][$repeatable_field['id']], array( $id, $i ) );
					echo '</p>';
				} // end each field
 ...
 */
function metaboxes_find_field_type( $needle, $haystack ) {
	foreach ( $haystack as $h )
		if ( isset( $h['type'] ) && $h['type'] == 'repeatable' )
			return metaboxes_find_field_type( $needle, $h['repeatable_fields'] );
 ...
 * @return  bool                whether or not the type is in the provided array
 */
function metaboxes_find_repeatable( $needle, $haystack ) {
	foreach ( $haystack as $h )
		if ( isset( $h['type'] ) && $h['type'] == 'repeatable' )
			return true;
		else
 ...
 */
function metaboxes_box_sanitize( $string, $function = 'sanitize_text_field' ) {
	switch ( $function ) {
		case 'intval':
 ...
 */
function metaboxes_array_map_r( $func, $meta, $sanitizer ) {
	$newMeta = array();
	$meta = array_values( $meta );
 ...
 */
class Metaboxes_Custom_Add_Meta_Box {
	var $id;
	var $title;
	var $fields;
	var $page;
	public function __construct( $id, $title, $fields, $page, $js, $template = null ) {
		...
		add_action( 'admin_enqueue_scripts', array( $this, 'metaboxes_admin_enqueue_scripts' ) );     
		add_action( 'admin_footer',  array( $this, 'metaboxes_admin_head' ) );
		add_action( 'add_meta_boxes', array( $this, 'metaboxes_add_box' ) );
		add_action( 'save_post',  array( $this, 'metaboxes_save_box' ));
	}
	/**
	 * adds scripts to the head for special fields with extra js requirements
	 */
	function metaboxes_admin_head() {
		if ( in_array( get_post_type(), $this->page ) && ( metaboxes_find_field_type( 'date', $this->fields ) || metaboxes_find_field_type( 'slider', $this->fields ) ) ) {
	 ...
	 */
	function metaboxes_admin_enqueue_scripts() {
		global $pagenow;
		if ( in_array( $pagenow, array( 'post-new.php', 'post.php' ) ) && in_array( get_post_type(), $this->page ) ) {
			// js
			$deps = array( 'jquery', 'jquery-migrate' );
			if ( metaboxes_find_field_type( 'date', $this->fields ) )
				$deps[] = 'jquery-ui-datepicker';
			if ( metaboxes_find_field_type( 'slider', $this->fields ) )
				$deps[] = 'jquery-ui-slider';
			if ( metaboxes_find_field_type( 'color', $this->fields ) )
				$deps[] = 'farbtastic';
				$deps[] = 'wp-color-picker';
			
			$deps[] = 'jquery';
				wp_enqueue_script( 'meta_box', metaboxes_custom_dir().'/js/metaboxes-scripts.js', $deps );
			if ( in_array( true, array(  metaboxes_find_field_type( 'coordinates', $this->fields ) ) ) ) {
				...
				wp_enqueue_script('google-maps',$mapsurl, $deps, false, true);
			}
			// Creates the footer things like icon menu
			// css
			$deps = array();
			wp_register_style( 'jqueryui', metaboxes_custom_dir().'/css/jqueryui.css' );
			if ( metaboxes_find_field_type( 'date', $this->fields ) || metaboxes_find_field_type( 'slider', $this->fields ) )
				$deps[] = 'jqueryui';
			if ( metaboxes_find_field_type( 'color', $this->fields ) )
				$deps[] = 'farbtastic';
			wp_enqueue_style( 'meta_box', metaboxes_custom_dir().'/css/meta_box.css', $deps );
	...
	function metaboxes_add_box() {
		...
		*/
		...
		foreach ( $this->page as $page ) {
			add_meta_box( $this->id, $this->title, array( $this, 'metaboxes_callback' ), $page, 'normal', 'high' );
		}
	}
				...
				if ( $field['type'] == 'section' ) {
					...
					if($field['type'] == 'chapter'){
						...
						echo'</th>
								...
								echo metaboxes_custom_meta_box_field( $field, $meta );
						echo    '<td>';
					}
					echo '</tr>';
	 ...
	 */
	function metaboxes_save_box( $post_id ) {
		...
		foreach ( $this->fields as $field ) {
			...
			if( in_array( $field['type'], array( 'tax_select', 'tax_checkboxes' ) ) ) {
				...
				if ( is_array( $term ) ){
					$term = metaboxes_array_map_r( 'metaboxes_box_sanitize', $new, $sanitizer );
				}
				else {
					$term = metaboxes_box_sanitize( $new, $sanitizer );
				}
				if ( isset( $_POST[$field['id']] ) ) {
					update_post_meta( $post_id, $field['id'], $term );
				}
			}
			if( in_array( $field['type'], array( 'tax_select_disassociated') ) ) {
				...
				if ( isset( $new ) && $new != $old ) {
					$sanitizer = isset( $field['sanitizer'] ) ? $field['sanitizer'] : 'sanitize_text_field';
					if ( is_array( $new ) )
						$new = metaboxes_array_map_r( 'metaboxes_box_sanitize', $new, $sanitizer );
					else
						$new = metaboxes_box_sanitize( $new, $sanitizer );
